...
interface RevenueData {
    property_id: string;
    /** Half-up rounded dollars as string (e.g. "2250.00") */
    total_revenue: string | number;
    /** Optional exact cents for display without float drift */
...
function amountForDisplay(data: RevenueData): number {
    if (
        typeof data.total_revenue_minor_units === 'number' &&
        Number.isFinite(data.total_revenue_minor_units)
    ) {
        return data.total_revenue_minor_units / 100;
    }
    const raw =
        typeof data.total_revenue === 'string'
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({
    ...
    const displayTotal = amountForDisplay(data);
    const rawNumeric =
        typeof data.total_revenue === 'string'
            ? parseFloat(data.total_revenue)
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                ...
                <div className="mt-4 h-6">
                    {showRaw &&
                        data.total_revenue_minor_units === undefined &&
                        Math.abs(rawNumeric - displayTotal) > 0.000001 && (
